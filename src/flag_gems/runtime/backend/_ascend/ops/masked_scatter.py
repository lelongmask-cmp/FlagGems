...
def masked_scatter_impl(inp, mask, source, N):
    ...
    with torch_device_fn.device(inp.device):
        ...
        # The total is summed from the CPU counts instead of mask.sum(): under
        # use_gems the latter dispatches to the flag-gems sum kernel, which
        # overflows UB for large tensors and stalls on a device reduction.
        if int(counts_cpu.sum()) == 0:
            return inp
